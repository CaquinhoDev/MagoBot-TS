...
export const baileys = {
  ...
  skipUnreadMessages: JSON.parse(
    process.env.SB_SKIP_UNREAD || "false"
  ) as boolean,
  storeAutoDelete: JSON.parse(
    process.env.SB_STORE_AUTO_DELETE || "true"
  ) as boolean,
...
// Bot config
export const bot = {
  name: process.env.SB_NAME || "MagoBot",
  sessionId: process.env.WA_SESSION_ID || "magobot",
  setup: JSON.parse(process.env.SB_SETUP || "false") as boolean,
  status: process.env.SB_STATUS || "Developed by CaquinhoDev",
  donationLink: process.env.SB_DONATION || "11913372146",
  ...
  lastSeenPrivacy:
    (process.env.SB_LAST_SEEN_PRIVACY as WAPrivacyValue) || "all",
  onlinePrivacy:
    (process.env.SB_ONLINE_PRIVACY as WAPrivacyOnlineValue) || "all",
  setProfilePic: JSON.parse(
    process.env.SB_SET_PROFILE_PIC || "false"
  ) as boolean,
  profilePicPrivacy:
    (process.env.SB_PROFILE_PIC_PRIVACY as WAPrivacyValue) || "all",
  statusPrivacyValue:
    (process.env.SB_STATUS_PRIVACY as WAPrivacyValue) || "all",
  readReceiptsPrivacy:
    (process.env.SB_READ_RECEIPTS_PRIVACY as WAReadReceiptsValue) || "all",
  groupsAddPrivacy:
    (process.env.SB_GROUPS_ADD_PRIVACY as WAPrivacyGroupAddValue) ||
    "contact_blacklist",
  defaultDisappearingMode: parseInt(
    process.env.SB_DEFAULT_DISAPPEARING_MODE || "604800"
  ) as number,
  ...
  forceCommunity: JSON.parse(
    process.env.SB_FORCE_COMMUNITY || "false"
  ) as boolean,
  community: process.env.SB_COMMUNITY,
  logsGroup: process.env.SB_LOGS_GROUP,
  prefixes: process.env.SB_PREFIXES
    ? process.env.SB_PREFIXES.split(";")
    : ["!"],
  ...
  groups: process.env.SB_GROUPS?.replaceAll(" ", "").split(";") || [], // TODO - Get from community?
};
