...
export const getProjectHomepage = () => {
  const info = getProjectInfo();
  return info
    ? info.homepage.replace("#readme", "")
    : "https://github.com/caquinhodev/magobotts";
};
...
const getJsonFromUrl = async (url: string) => {
  try {
    ...
    if (error instanceof Error) {
      ...
      logger.error(
        "Error fetching json from url: An unexpected error occurred"
      );
    }
  }
  return false;
...
export const getProjectLatestVersion = async () => {
  try {
    const versionUrl =
      "https://raw.githubusercontent.com/caquinhodev/magobotts/main/package.json"; // don't change it
    ...
    if (error instanceof Error) {
      ...
      logger.error(
        "Error fetching version from json: An unexpected error occurred"
      );
    }
  }
};
    ...
    if (stats.size > maxSizeInBytes) {
      ...
      fs.unlink(filePath, (err) => {
        if (err) {
          ...
          logger.info(
            `Store file deleted successfully as it was larger than ${baileys.storeMaxFileSize} MB`
          );
        }
      });
    }
